...
REMOTE_CONTROLLER_IP = "192.168.56.109"
...
class MultipathTopo(Topo):
    ...
    def __init__(self,j):
        ...
        #topo_metric3 = []
        for i in range(2):          
            topo_metric1.append(round(random.uniform(3, 15)/10, 2))
            topo_metric1.append(random.randint(0, 200))
            topo_metric1.append(random.randint(0, 20)/10)
        """
        for j in range(3):          
        ...
        """
        with open('./data/topoinfo_2links.csv', 'a') as f:
            writer = csv.writer(f, delimiter=',')
            writer.writerow((
            str(topo_metric1[0]), str(topo_metric1[1]), str(topo_metric1[2]), str(topo_metric1[3]),str(topo_metric1[4]), str(topo_metric1[5]), str(j)
...
def video_trans(h1, h2):
    ...
    #os.system("cp output.ts ./output/video/2link_received_video/received_video.ts")
    #os.remove("output.ts")
...
def ttest(x):
    setLogLevel('info')
    print ("this is %d round"%x)
  #  simpleTest()
    topo = MultipathTopo(x)
